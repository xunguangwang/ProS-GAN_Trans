...
# description of data
parser.add_argument('--dataset_name', dest='dataset', default='ImageNet', choices=['CIFAR-10', 'ImageNet', 'FLICKR-25K', 'NUS-WIDE', 'COCO'], help='name of the dataset')
...
parser.add_argument('--target_model', dest='target_model', default='NUS-WIDE_DPSH_ResNet18_32.pth', help='targeted model')
...
dset_database = HashingDataset(args.data_dir + args.dataset,
                               args.database_file, args.database_label)
dset_train = HashingDataset(args.data_dir + args.dataset, args.train_file,
                            args.train_label)
dset_test = HashingDataset(args.data_dir + args.dataset, args.test_file,
                           args.test_label)
num_database, num_test, num_train = len(dset_database), len(dset_test), len(
    dset_train)
...
query_images = torch.zeros([num_test, 3, 224, 224])
for data in test_loader:
    query_img, _, index = data
    query_images[index, :] = query_img
...
if args.train:
    if args.load:
        model.load_model()
    model.train(train_loader, target_labels, database_loader, database_labels,
                num_database, num_train, num_test)

if args.test:
    model.load_model()
    model.test(target_labels, database_loader, test_loader, database_labels,
               test_labels, num_database, num_test)

if args.transfer:
    model.load_model()
    model.transfer_test(target_labels, database_loader, test_loader,
