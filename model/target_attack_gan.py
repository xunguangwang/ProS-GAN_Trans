...
class TargetAttackGAN(nn.Module):
    def __init__(self, args):
        ...
        self.model_name = '{}_{}_{}_{}'.format(args.dataset, args.hash_method,
                                               args.backbone, args.bit)
        self.lr = args.lr
        self.args = args
    ...
    def _build_model(self):
        ...
        self.label_net = nn.DataParallel(LabelNet(self.bit,
    ...
    def train(self, train_loader, target_labels, database_loader,
              database_labels, num_database, num_train, num_test):
        ...
        self.schedulers = [
            get_scheduler(opt, self.args) for opt in self.optimizers
        ]

        # label net
        if os.path.exists(
                os.path.join(self.args.save,
                             'labelnet_{}.pth'.format(self.model_name))):
            self.load_labelnet()
        else:
            self.train_label_net(train_loader, target_labels, num_train)
        ...
        for i in range(num_test):
            ...
            _, target_hash_l, __ = self.label_net(
                batch_target_label.cuda().float())
            qB[i, :] = torch.sign(target_hash_l.cpu().data).numpy()[0]

        database_code_path = os.path.join(
            'log', 'database_code_{}.txt'.format(self.model_name))
        if os.path.exists(database_code_path):
            ...
            dB = dB.numpy()
        map_ = CalcMap(qB, dB, targeted_labels, database_labels.numpy())
        ...
        for epoch in range(self.args.epoch_count, total_epochs):
            print('\nTrain epoch: {}, learning rate: {:.7f}'.format(
                epoch, self.lr))
            for i, data in enumerate(train_loader):
                ...
                batch_target_label = target_labels.index_select(
                    0, torch.from_numpy(select_index)).cuda()

                feature, target_hash_l, _ = self.label_net(batch_target_label)
                ...
                if i % 3 == 0:
                    ...
                    fake_d_loss = self.criterionGAN(fake_d, batch_target_label,
                                                    False)
                ...
                fake_g_loss = self.criterionGAN(fake_g_d, batch_target_label,
                                                True)
                ...
                # adaptive
                logloss = (logloss - 2) * logloss
                logloss = torch.mean(logloss)
                logloss = (logloss + 1) / 1.0
                # vallina
                # logloss = torch.mean(logloss)
                # logloss = (-logloss + 1) / 1.0

                # backpropagation
                g_loss = self.rec_w * reconstruction_loss + logloss + self.dis_w * fake_g_loss
                g_loss.backward()
                optimizer_g.step()

                if i % self.args.sample_freq == 0:
                    self.sample(
                        fake_g, '{}/{}/'.format(self.args.sample,
                                                self.model_name),
                        str(epoch) + '_' + str(i) + '_fake')
                    self.sample(
                        real_input, '{}/{}/'.format(self.args.sample,
                                                    self.model_name),
                ...
                if i % self.args.print_freq == 0:
                    print(
                        ...
                        .format(i, fake_g_loss, d_loss, logloss,
                                reconstruction_loss))
    ...
    def train_label_net0(self, train_loader, target_labels, num_train):
        optimizer_l = torch.optim.Adam(self.label_net.parameters(),
                                       lr=self.args.lr,
                                       betas=(0.5, 0.999))
        ...
        for epoch in range(epochs):
            for i in range(steps):
                ...
                _, target_hash_l, label_pred = self.label_net(
                    batch_target_label)
                theta_x = target_hash_l.mm(Variable(B).t()) / 2
                logloss = (Variable(S.cuda()) * theta_x -
                           log_trick(theta_x)).sum() / (num_train * batch_size)
                logloss = -logloss
                regterm = (torch.sign(target_hash_l) - target_hash_l
                           ).pow(2).sum() / (num_train * batch_size)
                classifer_loss = 100 * criterion_l2(label_pred,
                                                    batch_target_label)
                ...
                if i % self.args.sample_freq == 0:
                    print(
                        'epoch: {:2d}, step: {:3d}, lr: {:.5f}, logloss:{:.5f}, regterm: {:.5f}, l2_loss: {:.7f}'
                        .format(epoch, i,
                                scheduler.get_last_lr()[0], logloss, regterm,
                                classifer_loss))
                scheduler.step()

        self.save_labelnet()

    def train_label_net(self, train_loader, target_labels, num_train):
        optimizer_l = torch.optim.Adam(self.label_net.parameters(),
                                       lr=self.args.lr,
                                       betas=(0.5, 0.999))
        ...
        for epoch in range(epochs):
            for i in range(steps):
                ...
                _, target_hash_l, label_pred = self.label_net(
                    batch_target_label)
                sp, sn = similarity(target_hash_l, B, batch_target_label,
                                    self.train_labels.cuda(), self.bit)
                logloss = circle_loss(sp, sn) / (batch_size)
                regterm = (torch.sign(target_hash_l) -
                           target_hash_l).pow(2).sum() / (1e4 * batch_size)
    ...
    def save_labelnet(self):
        torch.save(
            self.label_net.module.state_dict(),
            os.path.join(self.args.save,
                         'labelnet_{}.pth'.format(self.model_name)))

    def save_generator(self):
        torch.save(
            self.generator.module.state_dict(),
            os.path.join(
                self.args.save,
                'generator_{}_{}_{}.pth'.format(self.model_name, self.rec_w,
                                                self.dis_w)))

    def load_labelnet(self):
        self.label_net.module.load_state_dict(
            torch.load(
                os.path.join(self.args.save,
                             'labelnet_{}.pth'.format(self.model_name))))
    ...
    def load_generator(self):
        self.generator.module.load_state_dict(
            torch.load(
                os.path.join(
                    self.args.save,
                    'generator_{}_{}_{}.pth'.format(self.model_name,
                                                      self.rec_w,
                                                      self.dis_w))))
    ...
    def sample(self, image, sample_dir, name):
        ...
        image = (image + 1) / 2
        # min_ = torch.min(image).item()
        # max_ = torch.max(image).item()
        # image = (image - min_)/max_
        image = transforms.ToPILImage()(image)
        image.convert(mode='RGB').save(os.path.join(sample_dir, name + '.png'),
                                       quality=100)

    def test(self, target_labels, database_loader, test_loader,
             database_labels, test_labels, num_database, num_test):
        self.hashing_model.eval()
        self.label_net.eval()
        self.generator.eval()
        qB = np.zeros([num_test, self.bit])
        ...
        for it, data in enumerate(test_loader):
            ...
            feature = self.label_net(batch_target_label.cuda())[0]
            target_fake, mix_image = self.generator(data_input, feature)
            target_fake = (target_fake + 1) / 2
            data_input = (data_input + 1) / 2

            perceptibility += F.mse_loss(data_input,
                                         target_fake).data * data_ind.size(0)

            self.sample(
                2 * target_fake - 1,
                'result/{}/{}/'.format(self.args.sample, self.model_name),
                str(it) + '_fake')
            self.sample(
                2 * data_input - 1,
                'result/{}/{}/'.format(self.args.sample, self.model_name),
            ...
            qB[data_ind.numpy(), :] = torch.sign(
                target_hashing.cpu().data).numpy()

        end = time.time()
        print('Running time: %s Seconds' % (end - self.start))
        np.savetxt(os.path.join(
            'log', 'test_code_{}_gan_{}.txt'.format(self.args.dataset,
                                                    self.bit)),
                   qB,
                   fmt="%d")
        np.savetxt(os.path.join(
            'log', 'target_label_{}_gan_{}.txt'.format(self.args.dataset,
                                                       self.bit)),
                   targeted_labels,
                   fmt="%d")
        database_code_path = os.path.join(
            'log', 'database_code_{}.txt'.format(
                self.args.target_model.split('.')[0]))
        if os.path.exists(database_code_path):
            dB = np.loadtxt(database_code_path, dtype=np.float)
        else:
            dB = self.generate_hash_code(database_loader, num_database)
            dB = dB.numpy()
        print('perceptibility: {:.7f}'.format(
            torch.sqrt(perceptibility / num_test)))
        map_ = CalcMap(qB, dB, targeted_labels, database_labels.numpy())
        print('t_MAP(retrieval database): %3.5f' % (map_))

    def transfer_test(self, target_labels, database_loader, test_loader,
                      database_labels, test_labels, num_database, num_test,
                      target_model_path):
        self.hashing_model = torch.load(
            os.path.join(self.args.save, target_model_path))
        self.bit = self.args.t_bit
        self.test(target_labels, database_loader, test_loader, database_labels,
                  test_labels, num_database, num_test)
